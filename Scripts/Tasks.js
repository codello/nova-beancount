...
exports.beancount = {
    async resolveTaskAction(context) {
        ...
        if (context.action == Task.Build) {
            ...
            if (!journal) {
                throw "Journal path not specified"
            }
            ...
            if (reverse) {
                script += ` | cat - ${util.escapeShell(journal)} > /tmp/bc-journal.tmp && mv /tmp/bc-journal.tmp ${util.escapeShell(journal)}`;
            } else {
                script += ` >> ${util.escapeShell(journal)}`
            }
